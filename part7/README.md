## Parte 7
## Tarea 7.1: usar el gancho de campo
...
## Tarea 7.3: Solucionando el problema de la dispersión
...
## Tarea 7.4: usarAnécdotas, paso 1
...
## Tarea 7.5: usarAnécdotas, paso 2
  ...
  return (
    <Router>
      <div>
        ...
        <Routes>
          <Route path="/" element={<AnecdoteList anecdotes={anecdotes} />} />
          {/* Pasamos directamente la función del hook al componente */}
...
## Tarea 7.6: usarAnécdotas, paso 3
...
## Tarea 7.7: Frontend y backend en el mismo repositorio
server como backend, funciona con npm start
client como frontend, funciona con npm run build
en un monorepo
...
("Ruta al dist:", path.resolve(__dirname, '../client/dist'));
...
anecdoteRouter.get('/', async (req, res) => {
    try {
        ("Recibiendo petición de anécdotas...");
        
        res.json({ message: "Esto es una prueba de que el servidor funciona" })
    } catch (error) {
        console.error(error)
...
<html lang="en">
  <head>
    ...
    <title>client</title>
    <script type="module" crossorigin src="/assets/index-z35pfZ1W.js"></script>
  </head>
  <body>
    <div id="root"></div>
...
```
const TestComponent = () => {
    throw new Error('¡Esto es un error de renderizado!')
    return <div>Si ves esto, algo falló mal.</div>
}
...
## Tarea 7.10: Formateo automático de código
...
settings.json (dice 1 problema en este archivo):
...
.prettierignore (6 problemas en este archivo):

```
# Carpetas de compilación y dependencias
node_modules
dist
build
.git
...
# Archivos de configuración y lock
package-lock.json
package.json
yarn.lock
pnpm-lock.yaml
...
export default defineConfig([
  globalIgnores(['dist', '.prettierignore', '**/*.json']),
  {
    files: ['**/*.{js,jsx}'],
    extends: [
...
```
PS C:...\fullstackopen2026-AH\part7\monorepo\client> npm run lint

> client@0.0.0 lint
> eslint .


C:...\fullstackopen2026-AH\part7\monorepo\client\src\components\ErrorBoundary.jsx
  9:35  error  'error' is defined but never used  no-unused-vars

C:...\fullstackopen2026-AH\part7\monorepo\client\src\components\TestComponent.jsx
...
## Tarea 7.11: Zustand, Paso 1

client/src/services/anecdotes.js:
...
client/src/components/AnecdoteForm.jsx:

```
import useNotificationStore from '../store/notificationStore'
import anecdoteService from '../services/anecdotes'

const AnecdoteForm = ({ updateList }) => {
    ...
    setNotification(`Nueva anécdota creada: "${content}"`)
    updateList()
    }
...
server/controllers/anecdotes.js:
...
anecdoteRouter.get('/', (req, res) => {
  res.json(anecdotes)
})

anecdoteRouter.post('/', (req, res) => {
  const newAnecdote = {
...
function App() {
  ...
  return (
    <div>
      <h2>Front and back in same project!</h2>
      <Notification />
      <AnecdoteForm updateList={() => anecdoteService.getAll().then(setAnecdotes)} />

      <h3>Anécdotas:</h3>
      {anecdotes.map(a => <p key={a.id}>{a.content}</p>)}
...
function App() {
  ...
  if (result.isLoading) return <div>cargando datos...</div>
  if (result.isError) return <div>error al cargar los datos</div>
  ...
  return (
    <div>
      ...
      <Notification />
      {/* Pasamos la mutación como prop al formulario */}
      <AnecdoteForm mutate={newAnecdoteMutation.mutate} />
...
## Tarea 7.13: Zustand, Paso 3
No pude conectar mongodb atlas, lo hice en el servidor.
...
const remove = async (id) => {
    ('--- INICIO PETICIÓN DELETE ---')
    ('URL final:', `${baseUrl}/${id}`)
    ...
    try {
        ...
        console.error('ERROR EN AXIOS:', error.response ? error.response.data : error.message)
        throw error
    }
}
...
function App() {
  ...
  // 1. Declaración única para borrar
  ...
  // 2. Declaración necesaria para votar (que faltaba)
...
controllers/anecdotes.js:

```
const anecdoteRouter = require('express').Router()

let anecdotes = [
  { id: 1, content: 'Si funciona, no lo toques', votes: 0 },
  { id: 2, content: 'El código es poesía, pero sin rima', votes: 0 }
]
...
anecdoteRouter.put('/:id', (req, res) => {
  const id = Number(req.params.id)
  const { votes } = req.body
  
  anecdotes = anecdotes.map(a => a.id === id ? { ...a, votes } : a)
  res.json(anecdotes.find(a => a.id === id))
})

anecdoteRouter.delete('/:id', async (req, res) => {
  const id = Number(req.params.id)
  ("Servidor: intentando borrar ID:", id)
  
  anecdotes = anecdotes.filter(a => a.id !== id)
  
  ("Servidor: lista actual tras borrar:", anecdotes)
  res.status(204).end()
})
...
const getAll = async () => {
    const response = await axios.get(baseUrl)
    return response.data.map(anecdote => ({
    ...anecdote,
    id: anecdote.id || anecdote._id
    }))
}
...
import { QueryClient, QueryClientProvider } from '@tanstack/react-query'
import App from './App'
import { UserProvider } from './context/UserProvider' // AQUÍ sí va
...
function App() {
  ...
  // Definiciones únicas de mutaciones y queries
  const deleteMutation = useMutation({
    mutationFn: anecdoteService.remove,
      ...
      {result.data.map(anecdote => (
        <div key={anecdote.id}> 
          ...
          <div>
            ...
            <button onClick={() => handleVote(anecdote)}>vote</button>
            <button onClick={() => {
              ("ID que intento borrar:", anecdote.id);
              ("Objeto completo:", anecdote);
...
## Tarea 7.15: Limpieza del código
...
## Tarea 7.16: Vista de los usuarios
...
usersRouter.post('/', async (request, response) => {
    ...
    if (!password || password.length < 3) {
        return response.status(400).json({ error: 'La contraseña es muy corta o falta' })
    }
...
usersRouter.get('/fix-data', async (request, response, next) => {
    try {
        ...
        for (const user of users) {
            const userBlogs = blogs.filter(b => 
                ...
                b.author.toLowerCase() === user.username.toLowerCase()
            )
            ...
            user.blogs = userBlogs.map(b => b._id)
            await user.save()
        }
        
        response.send("¡Sincronización forzada completada!")
    } catch (error) {
        next(error)
    }
})
...
usersRouter.get('/fix-data', async (request, response, next) => {
    try {
        ...
        (`Blogs encontrados: ${blogs.length}`)
        ...
        for (const user of users) {
            ...
            (`Usuario ${user.username} tiene ${userBlogs.length} blogs asociados`)
        ...
        response.send("¡Proceso terminado! Revisa tu terminal para ver si encontró coincidencias.")
    } catch (error) {
        next(error)
    }
})
...
const BlogList = () => {
    ...
    if (result.isLoading) return <div>Cargando blogs...</div>
...
client/src/components/BlogForm.jsx:
...
const BlogForm = () => {
    ...
    const addBlog = (event) => {
        event.preventDefault()
            newBlogMutation.mutate({ title, author })
    }
...
const createNew = (blogObject) => axios.post(baseUrl, blogObject).then(res => res.data)

const update = async (blog) => {
    const { id, _id, __v, ...dataToUpdate } = blog
    const response = await axios.put(`${baseUrl}/${id}`, dataToUpdate)
    return response.data
}
...
## Tarea 7.17: Vista individual del usuario
...
client/src/components/UserView.jsx:
...
## Tarea 7.18: Comentarios, paso 1

client/services/blogs.js:
